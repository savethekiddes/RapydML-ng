			...
			if indent > self.verbatim_indent:
				...
				if not self.creating_method:
					if verbatim_properties[2] == SINGLE_LINE:
						...
						verbatim_command = 'echo {}'.format(subprocess.quote(self.verbatim_buffer)) + verbatim_properties[3]
						result = subprocess.run(verbatim_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, text=True, check=True)
						if result[1]:
